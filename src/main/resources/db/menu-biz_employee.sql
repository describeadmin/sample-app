...
--    'employee/index' 对应 frontend 的 src/views/employee/index.vue。
...
INSERT INTO sys_menu (parent_id, menu_name, menu_type, perm_code, path, component,
                      icon, sort, visible, create_time, update_time, deleted, version)
SELECT m.id, '员工', 'MENU', 'employee:list', '/employee', 'employee/index', 'lucide:table', 1, 1,
       NOW(), NOW(), 0, 0
FROM sys_menu m
WHERE m.path = '/biz' AND m.parent_id = 0 AND m.deleted = 0
  AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE perm_code = 'employee:list' AND deleted = 0);

INSERT INTO sys_menu (parent_id, menu_name, menu_type, perm_code, path, component,
                      icon, sort, visible, create_time, update_time, deleted, version)
SELECT m.id, '新增', 'BUTTON', 'employee:add', NULL, NULL, NULL, 1, 1, NOW(), NOW(), 0, 0
FROM sys_menu m
WHERE m.perm_code = 'employee:list' AND m.deleted = 0
  AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE perm_code = 'employee:add' AND deleted = 0);

INSERT INTO sys_menu (parent_id, menu_name, menu_type, perm_code, path, component,
                      icon, sort, visible, create_time, update_time, deleted, version)
SELECT m.id, '编辑', 'BUTTON', 'employee:edit', NULL, NULL, NULL, 2, 1, NOW(), NOW(), 0, 0
FROM sys_menu m
WHERE m.perm_code = 'employee:list' AND m.deleted = 0
  AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE perm_code = 'employee:edit' AND deleted = 0);

INSERT INTO sys_menu (parent_id, menu_name, menu_type, perm_code, path, component,
                      icon, sort, visible, create_time, update_time, deleted, version)
SELECT m.id, '删除', 'BUTTON', 'employee:remove', NULL, NULL, NULL, 3, 1, NOW(), NOW(), 0, 0
FROM sys_menu m
WHERE m.perm_code = 'employee:list' AND m.deleted = 0
  AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE perm_code = 'employee:remove' AND deleted = 0);
...
WHERE r.role_code = 'ADMIN' AND r.deleted = 0
  AND m.deleted = 0
  AND (m.perm_code LIKE 'employee:%' OR m.path = '/biz')
  AND NOT EXISTS (
